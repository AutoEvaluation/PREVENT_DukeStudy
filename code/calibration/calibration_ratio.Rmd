...
compute_xgb_recalibrated_risks <- function(relax, strict, year, outcome, n_iter = 50) {
  ...
  # 3) final retrain on all strict data (no sex stratification)
  df_tr <- strict
  
  # 80/20 split
  set.seed(300)
  split_i <- createDataPartition(df_tr[[col_event]], p=0.8, list=FALSE)
  tr_i    <- split_i
  vl_i    <- setdiff(seq_len(nrow(df_tr)), tr_i)
  
  # inner CV random-search
  set.seed(400)
  inner_flds <- createFolds(df_tr[[col_event]][tr_i], k=10)
  best_score <- Inf
  best_pars  <- NULL
  ...
  for (j in seq_len(n_iter)) {
    ...
    cv_scores <- vapply(inner_flds, function(v_i) {
      tr_i_in <- setdiff(tr_i, v_i)
      
      t_tr  <- df_tr[[col_time]][tr_i_in]
      e_tr  <- df_tr[[col_event]][tr_i_in]
      lbl_tr<- t_tr * ifelse(e_tr==1,1L,-1L)
      dtr   <- xgb.DMatrix(as.matrix(df_tr[tr_i_in, ..features]), label=lbl_tr)
      
      t_vl  <- df_tr[[col_time]][v_i]
      e_vl  <- df_tr[[col_event]][v_i]
      lbl_vl<- t_vl * ifelse(e_vl==1,1L,-1L)
      dvl   <- xgb.DMatrix(as.matrix(df_tr[v_i, ..features]), label=lbl_vl)
  ...
  # train final on all df_tr
  t_all  <- df_tr[[col_time]]
  e_all  <- df_tr[[col_event]]
  lbl_all<- t_all * ifelse(e_all==1,1L,-1L)
  d_all  <- xgb.DMatrix(as.matrix(df_tr[, ..features]), label=lbl_all)
  
  mdl_all <- xgb.train(
    params               = best_pars,
    data                 = d_all,
    nrounds              = 1000,
    watchlist            = list(val = d_all),
    early_stopping_rounds= 20,
    verbose              = 0
  )
  
  # calibrate on df_tr
  preds_all    <- predict(mdl_all, d_all)
  km_all       <- survfit(Surv(t_all, e_all) ~ 1)
  S_obs_all    <- summary(km_all, times = year, extend = TRUE)$surv[1]
  cen_all      <- preds_all - mean(preds_all)
  S0_all       <- fminbnd(function(s) abs(mean(s^exp(cen_all)) - S_obs_all), 0, 1)$x
  
  # predict on relax for any missing values
  idx_rel <- which(is.na(relax[[col_xgb]]))
  if (length(idx_rel)) {
    dr <- xgb.DMatrix(as.matrix(relax[idx_rel, ..features]))
    pr <- predict(mdl_all, dr)
    cr <- pr - mean(pr)
    relax[idx_rel, (col_xgb) := 1 - S0_all^exp(cr)]
  }
...
calc_km_ratio <- function(df, year, outcome) {
  risk_cols <- list(
    ...
    sdoh    = paste0(year, "y_sdoh_", outcome)
  )
  ...
  colnames(result)[-1] <- c("prevent", "recal", "xgb", "local", "race", "sdoh")
  return(result)
}
```
...
for (y in years) {
  for (event in outcomes) {
    ...
    # Step 4: Save
    fname <- paste0("results/calibration_ratio_", y, "y_", event, "_across_models.csv")
...
for (y in years) {
  for (event in outcomes) {
    ...
    fname <- paste0("results/calibration_ratio_strict_", y, "y_", event, "_across_models.csv")
    write.csv(ratio_wide_df, file = fname, row.names = FALSE)
    final_ratios[[paste0(y, "_", event)]] <- ratio_wide_df
  }
}
...
```{r}
write.csv(strict, '../../data/strict_cohort2/all_data_with_mlm.csv', row.names = FALSE)
write.csv(relax, '../../data/relax_cohort2/all_data_with_mlm.csv', row.names = FALSE)
```
...
for (y in c(5, 10)) {
  for (event in c("cvd", "ascvd", "hf")) {
    for (model_name in names(models)) {
      ...
      out_path <- sprintf("results/calibration_ratio_%sy_%s_%s_across_cohorts.csv", y, event, model_name)
