...
data[, tc := ifelse(is.na(tc), median(tc, na.rm = TRUE), tc)]
data[, hdlc := ifelse(is.na(hdlc), median(hdlc, na.rm = TRUE), hdlc)]
data[, bmi := ifelse(is.na(bmi), median(bmi, na.rm = TRUE), bmi)]
data[, scre := ifelse(is.na(scre), median(scre, na.rm = TRUE), scre)]
data[, sbp := ifelse(is.na(sbp), median(sbp, na.rm = TRUE), sbp)]
data[, eGFR := ifelse(is.na(eGFR), median(eGFR, na.rm = TRUE), eGFR)]
```
...
# save
write.csv(data, '../../data/relax_cohort2/transformed_data.csv', row.names = FALSE)
```
...
# apply
data <- fread('../../data/relax_cohort2/transformed_data.csv')
...
```{r}
# save
write.csv(data, '../../data/relax_cohort2/all_data.csv', row.names = FALSE)
```
