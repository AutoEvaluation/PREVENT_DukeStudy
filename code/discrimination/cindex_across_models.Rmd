...
relax = fread('../../data/relax_cohort2/all_data_with_mlm.csv')
```
...
run_analysis <- function(relax) {
  # Define parameters
  models <- c("prevent", "xgb", "local", "race", "sdoh")
  model_names <- c("PREVENT", "MLM Recalibrated", "Duke Local", "Duke Local(Race)", "Duke Local(Race&SDOH)")
...
strict = fread('../../data/strict_cohort2/all_data_with_mlm.csv')
```
...
run_analysis <- function(strict) {
  ...
  for(event in events) {
    for(time_horizon in time_horizons) {
      ...
      # Save to CSV
      filename <- paste0("results/cindex_strict_", time_horizon, "y_", event, ".csv")
      write.csv(result_df, filename, row.names = FALSE)
